...
// ../../dist/index.js
import { readFileSync as L, statSync as P } from "fs";
import { join as H } from "path";
import { homedir as F } from "os";
import { createHash as V, randomBytes as X } from "crypto";
import { writeFileSync as Z, readFileSync as ee, mkdirSync as te, chmodSync as ie } from "fs";
import { dirname as se, join as re } from "path";
import { homedir as ne } from "os";
import { createHash as Re, randomBytes as $e, randomUUID as ve } from "crypto";
import { readFileSync as Se, writeFileSync as De, chmodSync as be, mkdirSync as Oe, rmdirSync as Me, statSync as Ae, unlinkSync as Ie, appendFileSync as Ce } from "fs";
import { join as Ne } from "path";
import { homedir as xe } from "os";
import { appendFileSync as qe, mkdirSync as Ge, readdirSync as ze, statSync as Ye, unlinkSync as Qe, writeFileSync as Ve } from "fs";
import { createHash as Xe } from "crypto";
import { homedir as Ze } from "os";
import { join as et } from "path";
import { statSync as tt, readFileSync as it } from "fs";
import { homedir as st } from "os";
import { join as rt } from "path";
import { watch as St, appendFileSync as Dt, statSync as bt, existsSync as Ot, renameSync as Mt, unlinkSync as At } from "fs";
import { homedir as It } from "os";
import { join as Ct } from "path";
import { readFileSync as ii, writeFileSync as si, mkdirSync as ri } from "fs";
import { dirname as ni } from "path";
import { randomUUID as ai } from "crypto";
import { readFileSync as ci, statSync as ui } from "fs";
import { spawn as Di, spawnSync as bi } from "child_process";
import { request as Oi } from "https";
import { randomBytes as Mi, createHash as Ai } from "crypto";
var e;
var t;
var i;
var s;
...
var O = (e2, t2) => v(e2, "name", { value: t2, configurable: true });
var M = ((e2) => "function" < "u" ? __require : typeof Proxy < "u" ? new Proxy(e2, { get: (e3, t2) => ("function" < "u" ? __require : e3)[t2] }) : e2)(function(e2) {
  if ("function" < "u")
    return __require.apply(this, arguments);
  throw Error('Dynamic require of "' + e2 + '" is not supported');
});
var A = (e2, t2) => function() {
  return e2 && (t2 = (0, e2[D(e2)[0]])(e2 = 0)), t2;
};
var I = (e2, t2) => {
  for (var i2 in t2)
    v(e2, i2, { get: t2[i2], enumerable: true });
};
var C = A({ "src/anthropic-endpoints.ts"() {
  i = (t = "https://" + (e = "api.anthropic.com")) + "/v1/messages", s = `${t}/v1/messages/count_tokens`, n = (r = "https://platform.claude.com") + "/v1/oauth/token", a = "https://claude.com/cai/oauth/authorize", o = `${r}/oauth/authorize`, l = `${r}/oauth/code/callback`, h = Object.freeze({ API_HOST: e, API_BASE: t, API_MESSAGES: i, API_MESSAGES_COUNT_TOKENS: s, PLATFORM_BASE: r, OAUTH_TOKEN_URL: n, OAUTH_AUTHORIZE_URL: a, OAUTH_CONSOLE_AUTHORIZE_URL: o, OAUTH_MANUAL_REDIRECT_URI: l });
} });
var N = A({ "src/anthropic-headers.ts"() {
  u = c = "2023-06-01", d = "Content-Type", f = "Authorization", p = "Accept", m = "User-Agent", y = "anthropic-version", g = "anthropic-beta", w = "anthropic-dangerous-direct-browser-access", _ = "x-app", T = "X-Claude-Code-Session-Id", E = "application/json", k = "application/json, text/event-stream", R = "text/html", $ = Object.freeze({ CONTENT_TYPE: d, AUTHORIZATION: f, ACCEPT: p, USER_AGENT: m, ANTHROPIC_VERSION: y, ANTHROPIC_BETA: g, ANTHROPIC_DANGEROUS_DIRECT_BROWSER_ACCESS: w, X_APP: _, X_CLAUDE_CODE_SESSION_ID: T, API_VERSION: c, CONTENT_TYPE_JSON: E, ACCEPT_JSON_SSE: k, CONTENT_TYPE_TEXT_HTML: R });
} });
var x = {};
I(x, { isBodyDumpEnabled: () => J, isDryRun: () => K, readFlags: () => B });
function U(e2) {
  let t2 = process.env[e2];
  return t2 === "1" || t2 !== "0" && undefined;
}
function B() {
  try {
    let e3 = P(W);
    if (e3.mtimeMs !== q || !z) {
      q = e3.mtimeMs, z = true;
      let t3 = JSON.parse(L(W, "utf8"));
      G = { ledger_dry_run: t3.ledger_dry_run ?? j.ledger_dry_run, dump_bodies_full: t3.dump_bodies_full ?? j.dump_bodies_full };
    }
  } catch {
    z || (G = { ...j });
  }
  let e2 = U("LEDGER_DRY_RUN"), t2 = U("CLAUDE_DUMP_BODIES_FULL");
  return { ledger_dry_run: e2 !== undefined ? e2 : G.ledger_dry_run, dump_bodies_full: t2 !== undefined ? t2 : G.dump_bodies_full };
}
function K() {
  return B().ledger_dry_run;
}
function J() {
  return B().dump_bodies_full;
}
var W;
var j;
var q;
var G;
var z;
var Y = A({ "src/runtime-flags.ts"() {
  W = H(F(), ".claude", "runtime-flags.json"), j = { ledger_dry_run: true, dump_bodies_full: true }, q = 0, G = { ...j }, z = false, O(U, "envOverride"), O(B, "readFlags"), O(K, "isDryRun"), O(J, "isBodyDumpEnabled");
} });
var Q = {};
I(Q, { getClaudeConfigDir: () => ae, getDefaultCredentialsPath: () => oe, oauthLogin: () => de });
function ae() {
  return (process.env.CLAUDE_CONFIG_DIR ?? re(ne(), ".claude")).normalize("NFC");
}
function oe() {
  return re(ae(), ".credentials.json");
}
function le() {
  return ue(X(32));
}
function he(e2) {
  return ue(V("sha256").update(e2).digest());
}
function ce() {
  return ue(X(32));
}
...
async function de(e2 = {}) {
  let t2 = e2.credentialsPath ?? oe(), i2 = le(), s2 = he(i2), r2 = ce(), { port: n2, waitForCode: a2, close: o2 } = await fe(r2, e2.port), l2 = `http://localhost:${n2}/callback`, h2 = e2.loginWithClaudeAi !== false ? we : ge, c2 = new URLSearchParams({ client_id: me, response_type: "code", scope: Ee, code_challenge: s2, code_challenge_method: "S256", state: r2, code: "true" });
  e2.loginHint && c2.set("login_hint", e2.loginHint), e2.loginMethod && c2.set("login_method", e2.loginMethod), e2.orgUUID && c2.set("orgUUID", e2.orgUUID);
  let u2, f2, p2 = `${h2}?${c2.toString()}&redirect_uri=${encodeURIComponent(l2)}`, m2 = `${h2}?${c2.toString()}&redirect_uri=${encodeURIComponent(Te)}`;
  e2.onAuthUrl ? e2.onAuthUrl(p2, m2) : (console.log(`
\uD83D\uDD10 Login to Claude
`), console.log(`Open this URL in your browser:
`), console.log(`  ${m2}
`)), e2.openBrowser !== false && pe(p2).catch(() => {});
  try {
    u2 = await a2, f2 = l2;
  } catch (e3) {
    throw o2(), e3;
  ...
  o2();
  let y2 = await fetch(_e, { method: "POST", headers: { [d]: E }, body: JSON.stringify({ grant_type: "authorization_code", code: u2, redirect_uri: f2, client_id: me, code_verifier: i2, state: r2 }) });
  if (!y2.ok) {
    let e3 = await y2.text();
    throw new Error(`Token exchange failed (${y2.status}): ${e3}`);
  }
  let g2 = await y2.json(), w2 = Date.now() + 1000 * g2.expires_in, _2 = { accessToken: g2.access_token, refreshToken: g2.refresh_token, expiresAt: w2, scopes: g2.scope?.split(" ") ?? [] }, T2 = {};
  try {
    T2 = JSON.parse(ee(t2, "utf8"));
  } catch {}
  T2.claudeAiOauth = _2;
  let k2 = se(t2);
  try {
    te(k2, { recursive: true });
  } catch {}
  return Z(t2, JSON.stringify(T2, null, 2), "utf8"), ie(t2, 384), console.log(`
\u2705 Login successful! Credentials saved to ${t2}
`), { accessToken: _2.accessToken, refreshToken: _2.refreshToken, expiresAt: _2.expiresAt, credentialsPath: t2 };
}
async function fe(e2, t2) {
  let i2, s2, r2 = new Promise((e3, t3) => {
    i2 = e3, s2 = t3;
  }), n2 = Bun.serve({ port: t2 ?? 0, async fetch(t3) {
    let r3 = new URL(t3.url);
    if (r3.pathname !== "/callback")
      return new Response("Not found", { status: 404 });
    let n3 = r3.searchParams.get("code"), a3 = r3.searchParams.get("state"), o2 = r3.searchParams.get("error");
    return o2 ? (s2(new Error(`OAuth error: ${o2} \u2014 ${r3.searchParams.get("error_description") ?? ""}`)), new Response("<html><body><h1>Login failed</h1><p>You can close this tab.</p></body></html>", { status: 400, headers: { [d]: R } })) : n3 && a3 === e2 ? (i2(n3), new Response(null, { status: 302, headers: { Location: `${ye}/oauth/code/success?app=claude-code` } })) : (s2(new Error("Invalid callback: missing code or state mismatch")), new Response("Invalid request", { status: 400 }));
  } }), a2 = setTimeout(() => {
    s2(new Error("Login timed out (5 minutes). Try again.")), n2.stop();
  }, 300000);
  return { port: n2.port, waitForCode: r2.finally(() => clearTimeout(a2)), close: O(() => {
    clearTimeout(a2), n2.stop();
  }, "close") };
}
...
var Te;
var Ee;
var ke = A({ "src/auth.ts"() {
  C(), N(), me = "9d1c250a-e61b-44d9-88ed-5944d1962f5e", ye = r, ge = o, we = a, _e = n, Te = l, O(ae, "getClaudeConfigDir"), O(oe, "getDefaultCredentialsPath"), Ee = ["user:profile", "user:inference", "org:create_api_key", "user:sessions:claude_code", "user:mcp_servers", "user:file_upload"].join(" "), O(le, "generateCodeVerifier"), O(he, "generateCodeChallenge"), O(ce, "generateState"), O(ue, "base64url"), O(de, "oauthLogin"), O(fe, "startCallbackServer"), O(pe, "tryOpenBrowser");
} });
...
var Ue = class extends Le {
  ...
  static {
    O(this, "CacheRewriteBlockedError");
  }
  code = "CACHE_REWRITE_BLOCKED";
};
var Be = { "claude-opus-4-7": { name: "Claude Opus 4.7", context: 1e6, defaultOutput: 64000, maxOutput: 128000, adaptiveThinking: true, cost: { input: 15, output: 75, cacheRead: 1.875, cacheWrite: 18.75 } }, "claude-opus-4-6": { name: "Claude Opus 4.6", context: 1e6, defaultOutput: 64000, maxOutput: 128000, adaptiveThinking: true, cost: { input: 15, output: 75, cacheRead: 1.875, cacheWrite: 18.75 } }, "claude-sonnet-4-6": { name: "Claude Sonnet 4.6", context: 1e6, defaultOutput: 32000, maxOutput: 128000, adaptiveThinking: true, cost: { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 } }, "claude-haiku-4-5-20251001": { name: "Claude Haiku 4.5", context: 200000, defaultOutput: 32000, maxOutput: 64000, adaptiveThinking: false, cost: { input: 0.8, output: 4, cacheRead: 0.08, cacheWrite: 1 } } };
var Ke = { defaultOutput: 32000, maxOutput: 128000, adaptiveThinking: false };
function Je(e2, t2) {
  if (typeof t2 == "number" && t2 > 0)
    return t2;
  let i2 = We(e2), s2 = i2?.maxOutput ?? Ke.maxOutput, r2 = i2?.defaultOutput ?? Ke.defaultOutput, n2 = process.env.CLAUDE_CODE_MAX_OUTPUT_TOKENS;
  if (n2) {
...
function We(e2) {
  ...
  for (let [e3, i2] of Object.entries(Be))
...
O(Je, "resolveMaxTokens"), O(We, "getModelMetadata"), O(je, "supportsAdaptiveThinking"), C();
var nt = { enabled: true, initialCalls: 3, ringRetentionMs: 7200000, ringMaxMb: 300, suspiciousContextSize: 5, suspiciousRetentionMs: 86400000, suspiciousMaxMb: 100, coldCwThreshold: 1e4, metadataRetentionMs: 604800000 };
var at = { cacheTtlMs: 300000, safetyMarginMs: 15000, intervalMs: 120000, intervalClampMin: 60000, retryDelaysMs: [2, 3, 5, 7, 10, 12, 15, 17, 20, 20, 20, 20, 20].map((e2) => 1000 * e2), rewriteWarnIdleMs: 300000, rewriteWarnTokens: 50000, healthProbeIntervalsMs: [3000, 5000, 7000, 1e4, 1e4, 1e4, 1e4, 1e4, 1e4, 1e4, 1e4, 1e4], healthProbeTimeoutMs: 3000, enabled: true, idleTimeoutMs: 1 / 0, minTokens: 2000, rewriteBlockEnabled: false, dump: nt, tokenRotationContextThreshold: 150000, tokenRotationPollIntervalMs: 30000, orgIdCacheTtlMs: 300000, tokenRotationLogMaxBytes: 10485760, tokenRotationLogRetentionDays: 7 };
var lt = rt(st(), ".claude", "keepalive.json");
var ht = process.env.CLAUDE_KEEPALIVE_CONFIG_PATH || lt;
var ct = 0;
var ut = null;
var dt = new Set;
function ft() {
  try {
    let e2 = tt(ht);
    return e2.mtimeMs === ct && ut ? null : (ct = e2.mtimeMs, dt.clear(), JSON.parse(it(ht, "utf8")));
  } catch {
    return null;
  }
}
function pt(e2, t2, i2, s2, r2) {
  if (e2 == null)
    return i2;
  let n2 = typeof e2 == "number" ? e2 : Number(e2);
  return Number.isFinite(n2) ? n2 < s2 || n2 > r2 ? (dt.has(t2) || (console.error(`[keepalive-config] ${t2}=${n2} out of range [${s2}, ${r2}] \u2014 clamping`), dt.add(t2)), Math.max(s2, Math.min(r2, n2))) : n2 : (dt.has(t2) || (console.error(`[keepalive-config] ${t2}=${JSON.stringify(e2)} is not a number \u2014 using fallback ${i2}`), dt.add(t2)), i2);
}
function mt(e2, t2, i2, s2 = 1, r2 = 30) {
  if (e2 == null)
    return i2;
  if (!Array.isArray(e2))
    return dt.has(t2) || (console.error(`[keepalive-config] ${t2} is not an array \u2014 using fallback`), dt.add(t2)), i2;
  let n2 = e2.map((e3) => typeof e3 == "number" ? e3 : Number(e3)).filter((e3) => Number.isFinite(e3) && e3 > 0);
  return n2.length < s2 || n2.length > r2 ? (dt.has(t2) || (console.error(`[keepalive-config] ${t2} length ${n2.length} out of [${s2}, ${r2}] \u2014 using fallback`), dt.add(t2)), i2) : n2;
}
function yt(e2, t2) {
...
function gt() {
  let e2 = ft();
  return e2 === null && ut ? ut : _t(e2 ?? null);
}
function wt() {
  return ct = 0, ut = null, gt();
}
function _t(e2) {
  let t2 = e2 === null ? "defaults" : Object.keys(e2).length > 0 ? "mixed" : "defaults", i2 = pt(e2?.cacheTtlMs ?? (typeof e2?.cacheTtlSec == "number" ? 1000 * e2.cacheTtlSec : undefined), "cacheTtlMs", at.cacheTtlMs, 60000, 7200000), s2 = pt(e2?.safetyMarginMs ?? (typeof e2?.safetyMarginSec == "number" ? 1000 * e2.safetyMarginSec : undefined), "safetyMarginMs", at.safetyMarginMs, 1000, 300000), r2 = Math.max(60000, Math.min(i2 / 2, 1800000)), n2 = pt(e2?.intervalMs ?? (typeof e2?.intervalSec == "number" ? 1000 * e2.intervalSec : undefined), "intervalMs", r2, 60000, i2 - s2 - 1000), a2 = at.intervalClampMin, o2 = Math.max(a2 + 1, i2 - s2 - 60000);
  n2 < a2 && (n2 = a2), n2 > o2 && (n2 = o2);
  let l2 = { cacheTtlMs: i2, safetyMarginMs: s2, intervalMs: n2, intervalClampMin: a2, intervalClampMax: o2, retryDelaysMs: mt(e2?.retryDelaysMs ?? (Array.isArray(e2?.retryDelaysSec) ? e2.retryDelaysSec.map((e3) => typeof e3 == "number" ? 1000 * e3 : NaN) : undefined), "retryDelaysMs", at.retryDelaysMs), rewriteWarnIdleMs: pt(e2?.rewriteWarnIdleMs ?? (typeof e2?.rewriteWarnIdleSec == "number" ? 1000 * e2.rewriteWarnIdleSec : undefined), "rewriteWarnIdleMs", Math.max(60000, i2 - s2), 1000, 86400000), rewriteWarnTokens: pt(e2?.rewriteWarnTokens, "rewriteWarnTokens", at.rewriteWarnTokens, 100, 1e6), healthProbeIntervalsMs: mt(e2?.healthProbeIntervalsMs, "healthProbeIntervalsMs", at.healthProbeIntervalsMs), healthProbeTimeoutMs: pt(e2?.healthProbeTimeoutMs, "healthProbeTimeoutMs", at.healthProbeTimeoutMs, 500, 60000), enabled: yt(e2?.enabled, at.enabled), idleTimeoutMs: e2?.idleTimeoutMs === null || e2?.idleTimeoutSec === null ? 1 / 0 : pt(e2?.idleTimeoutMs ?? (typeof e2?.idleTimeoutSec == "number" ? 1000 * e2.idleTimeoutSec : undefined), "idleTimeoutMs", at.idleTimeoutMs === 1 / 0 ? 86400000 : at.idleTimeoutMs, 0, 86400000), minTokens: pt(e2?.minTokens, "minTokens", at.minTokens, 1, 1e6), rewriteBlockEnabled: yt(e2?.rewriteBlockEnabled, at.rewriteBlockEnabled), dump: nt, tokenRotationContextThreshold: pt(e2?.tokenRotationContextThreshold, "tokenRotationContextThreshold", at.tokenRotationContextThreshold, 1000, 1e7), tokenRotationPollIntervalMs: pt(e2?.tokenRotationPollIntervalMs, "tokenRotationPollIntervalMs", at.tokenRotationPollIntervalMs, 5000, 3600000), orgIdCacheTtlMs: pt(e2?.orgIdCacheTtlMs, "orgIdCacheTtlMs", at.orgIdCacheTtlMs, 1e4, 86400000), tokenRotationLogMaxBytes: pt(e2?.tokenRotationLogMaxBytes, "tokenRotationLogMaxBytes", at.tokenRotationLogMaxBytes, 1024, 1073741824), tokenRotationLogRetentionDays: pt(e2?.tokenRotationLogRetentionDays, "tokenRotationLogRetentionDays", at.tokenRotationLogRetentionDays, 1, 3650), t: t2 };
  return ut = l2, l2;
}
function Tt() {
  return ht;
...
function Et() {
  return gt().cacheTtlMs;
}
function kt() {
  return gt().safetyMarginMs;
}
function Rt(e2) {
  let t2 = e2;
  if (!t2)
...
O(ft, "readRawConfig"), O(pt, "num"), O(mt, "numArray"), O(yt, "bool"), O(gt, "loadKeepaliveConfig"), O(wt, "reloadKeepaliveConfig"), O(_t, "_resolve"), O(Tt, "getConfigPath"), O(Et, "getCacheTtlMs"), O(kt, "getSafetyMarginMs"), O(Rt, "classifyError");
var $t = class _KeepaliveEngine {
  static {
    O(this, "KeepaliveEngine");
  }
  cacheTtlMs;
  safetyMarginMs;
  retryDelaysMs;
  ...
  async tick() {
    try {
      ...
      qe(et(Ze(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] KA_HEARTBEAT pid=${process.pid} state=${s3} regSize=${this.registry.size} idleSec=${t3} nextFireSec=${i3} cacheAgeSec=${e3 < 0 ? "na" : Math.round(e3 / 1000)} cacheTtlSec=${Math.round(this.cacheTtlMs / 1000)} intervalSec=${Math.round(this.config.intervalMs / 1000)}
`);
    } catch {}
    ...
    if (this.cacheWrittenAt > 0) {
      ...
      if (e3 > this.cacheTtlMs) {
        try {
          qe(et(Ze(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] KA_DISARM_CACHE_EXPIRED pid=${process.pid} cacheAgeSec=${Math.round(e3 / 1000)} cacheTtlSec=${Math.round(this.cacheTtlMs / 1000)} overSec=${Math.round((e3 - this.cacheTtlMs) / 1000)}
`);
        } catch {}
    ...
    if (e2.cacheTtlMs !== this.cacheTtlMs) {
      ...
      try {
        qe(et(Ze(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] CACHE_TTL_RELOADED pid=${process.pid} oldMs=${t3} newMs=${e2.cacheTtlMs} oldMin=${Math.round(t3 / 60000)} newMin=${Math.round(e2.cacheTtlMs / 60000)}
`);
      } catch {}
    }
    if (e2.safetyMarginMs !== this.safetyMarginMs) {
      ...
      try {
        qe(et(Ze(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] SAFETY_MARGIN_RELOADED pid=${process.pid} oldMs=${t3} newMs=${e2.safetyMarginMs}
`);
      } catch {}
    }
  ...
  logClearDiag(e2, t2) {
    try {
      ...
      qe(et(Ze(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] KA_CLEAR_DIAG pid=${process.pid} ${o2}
`);
    } catch {}
  }
  retryChain(e2, t2 = 0) {
  ...
  startHealthProbe(t2 = {}) {
    ...
    if (this.networkState = "degraded", i2 !== "degraded")
      try {
        this.config.onNetworkStateChange?.({ from: i2, to: "degraded", at: Date.now() });
      } catch {}
    let s2 = O(() => {
      let e2 = this.healthProbeIntervalsMs, t3 = e2[Math.min(this.healthProbeAttempt, e2.length - 1)];
      ...
      try {
        ...
        await new Promise((i4, s3) => {
          let r4 = t3({ host: e, port: 443 }), n3 = setTimeout(() => {
            r4.destroy(), s3(new Error("timeout"));
          }, this.healthProbeTimeoutMs);
          r4.once("connect", () => {
            clearTimeout(n3), r4.end(), i4();
          }), r4.once("error", (e2) => {
            clearTimeout(n3), s3(e2);
          });
        }), i3 = true;
      } catch {
        i3 = false;
      ...
      let r3 = this.networkState;
      this.networkState = "healthy";
      try {
        this.config.onNetworkStateChange?.({ from: r3, to: "healthy", at: Date.now() });
      } catch {}
      let n2 = this.cacheTtlMs - (Date.now() - this.cacheWrittenAt);
      this.registry.size > 0 && n2 > this.safetyMarginMs && this.tick();
    }, "probe");
    r2();
  }
  stopHealthProbe() {
  ...
  writeSnapshotDebug(e2, t2, i2) {
    try {
      let s2 = et(Ze(), ".claude", "snapshots");
      Ge(s2, { recursive: true });
      try {
        let e3 = Date.now() - _KeepaliveEngine.SNAPSHOT_TTL_MS;
        for (let t3 of ze(s2)) {
          let i3 = et(s2, t3);
          Ye(i3).mtimeMs < e3 && Qe(i3);
        }
      } catch {}
      this.snapshotCallCount++;
      let { messages: r2, system: n2, tools: a2 } = t2, o2 = typeof n2 == "string" ? n2 : JSON.stringify(n2), l2 = Xe("md5").update(o2).digest("hex").slice(0, 8), h2 = { ts: new Date().toISOString(), pid: process.pid, callNum: this.snapshotCallCount, model: e2, messages: r2?.length ?? 0, tools: a2?.length ?? 0, sysHash: l2, sysLen: o2.length, usage: { input: i2.inputTokens ?? 0, cacheRead: i2.cacheReadInputTokens ?? 0, cacheWrite: i2.cacheCreationInputTokens ?? 0 }, firstMsg: r2?.[0] ? { role: r2[0].role, contentLen: JSON.stringify(r2[0].content).length, contentHash: Xe("md5").update(JSON.stringify(r2[0].content)).digest("hex").slice(0, 8) } : null, lastMsg: r2?.length ? { role: r2[r2.length - 1].role, contentLen: JSON.stringify(r2[r2.length - 1].content).length } : null, toolsHash: a2?.length ? Xe("md5").update(JSON.stringify(a2.map((e3) => e3.name ?? "").join(","))).digest("hex").slice(0, 8) : null }, c2 = `${process.pid}-${Date.now()}.json`;
      if (Ve(et(s2, c2), JSON.stringify(h2, null, 2) + `
`), _KeepaliveEngine.DUMP_BODY || this.snapshotCallCount <= 3) {
        let e3 = et(s2, "bodies");
        Ge(e3, { recursive: true });
        let i3 = `${process.pid}-call${this.snapshotCallCount}-${Date.now()}.json`;
        Ve(et(e3, i3), JSON.stringify(t2, null, 2) + `
`);
      }
    } catch {}
  }
...
function vt(e2) {
  return !e2 || e2.length < 21 ? "" : e2.slice(13, 21);
}
O(vt, "tokenHint");
var Nt = class {
  constructor(e2, t2, i2) {
    this.credentialStore = e2, this.contextTokensProvider = t2, this.getConfig = i2, this.startWatcher(), this.startPollFallback();
  }
  static {
    O(this, "TokenRotationManager");
  }
  ...
  async checkPending() {
    ...
    if (this.contextTokensProvider)
      try {
        ...
        e2 = typeof t3 == "number" ? t3 : null;
      } catch (t3) {
        e2 = null, this.contextProviderThrew || (this.contextProviderThrew = true, this.logBestEffort(`[${new Date().toISOString()}] TOKEN_PROVIDER_THREW pid=${process.pid} error=${t3.message}`));
      }
    let t2 = this.getConfig().tokenRotationContextThreshold;
    if (e2 === null || e2 < t2) {
      await this.applyPending("context-drop");
      let e3 = await this.credentialStore.read();
      return e3 ? { action: "apply-now", credentials: e3, mode: "applied" } : { action: "no-pending" };
    }
    return { action: "continue-with-old", pending: this.pendingRotation };
  }
  async applyPending(e2, t2) {
    if (e2 === "forced-expired" && t2 === undefined)
    ...
    let o2 = e2 === "forced-expired" ? "forced" : "applied", l2 = e2, h2 = null;
    if (this.contextTokensProvider)
      try {
        let e3 = this.contextTokensProvider();
        h2 = typeof e3 == "number" ? e3 : null;
      } catch {
        h2 = null;
      }
    let c2 = { pid: process.pid, spawnDepth: 0, sessionId: null, oldHint: i2, newHint: s2, oldOrgId: r2, newOrgId: n2, contextTokens: h2, mode: o2, appliedAt: l2, forcedReason: t2 ?? null, detectedAt: new Date(a2).toISOString() };
    e2 === "forced-expired" ? this.logBestEffort(`[${new Date().toISOString()}] TOKEN_ROTATION_FORCED pid=${process.pid} forcedReason=${t2} oldHint=${i2} newHint=${s2}`) : this.logBestEffort(`[${new Date().toISOString()}] TOKEN_ROTATION_APPLIED pid=${process.pid} reason=${e2} oldHint=${i2} newHint=${s2} newOrgId=${n2 ?? "null"}`), this.emitEvent(c2);
  }
  hasPending() {
    return this.pendingRotation !== null;
  }
  setEventEmitter(e2) {
    this.eventEmitter = e2;
  }
  close() {
  ...
  startWatcher() {
    let e2 = this.credentialStore.path;
    if (typeof e2 == "string")
      try {
        this.watcher = St(e2, { persistent: false }, (e3) => {
          (e3 === "change" || e3 === "rename") && queueMicrotask(() => {
            this.detectRotation("fs.watch").catch(() => {});
          });
        }), this.watcher.on("error", (e3) => {
          this.logBestEffort(`[${new Date().toISOString()}] TOKEN_WATCHER_ERROR pid=${process.pid} error=${e3.message}`);
        });
      } catch (e3) {
        this.logBestEffort(`[${new Date().toISOString()}] TOKEN_WATCHER_INIT_FAILED pid=${process.pid} error=${e3.message}`);
      }
  }
  startPollFallback() {
    let e2 = this.getConfig().tokenRotationPollIntervalMs;
    this.pollTimer = setInterval(() => {
      this.closed || this.detectRotation("poll").catch(() => {});
    }, e2), this.pollTimer.unref();
  }
  async detectRotation(e2) {
    if (this.closed)
      return;
    let t2;
    try {
      ...
      return void this.logBestEffort(`[${new Date().toISOString()}] TOKEN_DETECT_READ_FAILED pid=${process.pid} source=${e2} error=${t3.message}`);
    }
    if (!t2)
      return;
    let i2 = vt(t2.accessToken);
    if (i2 === this.lastSeenHint)
      return;
    if (this.lastSeenHint === "")
      return this.lastSeenHint = i2, void (this.orgIdCache = { orgId: this.extractOrgId(t2.refreshToken), cachedAt: Date.now() });
    if (this.pendingRotation !== null && i2 === this.pendingRotation.newHint)
      return;
    if (this.pendingRotation !== null && i2 === this.pendingRotation.oldHint) {
      let e3 = this.pendingRotation;
      return this.pendingRotation = null, this.lastSeenHint = i2, void this.logBestEffort(`[${new Date().toISOString()}] TOKEN_ROTATION_CANCELLED pid=${process.pid} oldHint=${e3.oldHint} newHint=${e3.newHint} reason=revert`);
    }
    let s2 = this.lastSeenHint, r2 = i2, n2 = await this.getCachedOrgId(), a2 = this.extractOrgId(t2.refreshToken), o2 = null;
    if (this.contextTokensProvider)
      try {
        let e3 = this.contextTokensProvider();
        o2 = typeof e3 == "number" ? e3 : null;
      } catch (e3) {
        o2 = null, this.contextProviderThrew || (this.contextProviderThrew = true, this.logBestEffort(`[${new Date().toISOString()}] TOKEN_PROVIDER_THREW pid=${process.pid} error=${e3.message}`));
      }
    ...
    this.logBestEffort(`[${new Date().toISOString()}] ${c2} pid=${process.pid} source=${e2} oldHint=${s2} newHint=${r2} oldOrgId=${n2 ?? "null"} newOrgId=${a2 ?? "null"} contextTokens=${o2 ?? "null"}`), this.emitEvent(d2);
  }
  extractOrgId(e2) {
    if (!e2)
      return null;
    try {
      ...
      if (t2.length !== 3)
        return null;
      let i2 = Buffer.from(t2[1], "base64url").toString("utf8"), s2 = JSON.parse(i2);
  ...
  async getCachedOrgId() {
    let e2, t2 = this.getConfig().orgIdCacheTtlMs, i2 = Date.now();
    if (this.orgIdCache && i2 - this.orgIdCache.cachedAt < t2)
    ...
    let s2 = e2 ? this.extractOrgId(e2.refreshToken) : null;
    return this.orgIdCache = { orgId: s2, cachedAt: i2 }, s2;
  }
  appendRotationLog(e2) {
    try {
      let t2 = Ct(It(), ".claude", "token-rotation.log");
      Dt(t2, JSON.stringify(e2) + `
`), this.appendCallCount += 1, (this.appendCallCount === 1 || this.appendCallCount % 100 == 0) && this.maybeRotateLog();
    } catch {}
  }
  maybeRotateLog() {
    try {
      let e2 = this.getConfig(), t2 = Ct(It(), ".claude", "token-rotation.log");
      if (!Ot(t2) || bt(t2).size < e2.tokenRotationLogMaxBytes)
        return;
      let i2 = `${t2}.1`, s2 = `${t2}.2`;
      if (Ot(s2)) {
        let t3 = bt(s2).mtimeMs;
        if (Date.now() - t3 > 86400 * e2.tokenRotationLogRetentionDays * 1000)
          try {
            At(s2);
          } catch {}
      }
  ...
  emitEvent(e2) {
    if (this.eventEmitter)
      try {
        ...
        this.logBestEffort(`[${new Date().toISOString()}] TOKEN_EMITTER_THREW pid=${process.pid} error=${e3.message}`);
      }
    this.appendRotationLog(e2);
  }
  logBestEffort(e2) {
    try {
      Dt(Ct(It(), ".claude", "claude-max-debug.log"), e2 + `
`);
    } catch {}
  }
};
...
var Ut = 300000;
var Bt = 1200000;
var Kt = Ne(xe(), ".claude", ".refresh-cooldown");
var Jt = 1800000;
var Wt = "2.1.112";
var jt = { todowrite: "todo_write" };
var qt = Object.fromEntries(Object.entries(jt).map(([e2, t2]) => [t2, e2]));
function Gt(e2) {
  if (!e2?.length)
...
O(Gt, "remapToolNames"), O(zt, "unremapToolName");
var Yt = Ne(xe(), ".claude", ".token-refresh-lock");
async function Qt() {
  for (let e2 = 0;e2 < 30; e2++)
    try {
      return Oe(Yt), De(Ne(Yt, "pid"), `${process.pid}
${Date.now()}`), () => {
        try {
          Ie(Ne(Yt, "pid")), Me(Yt);
        } catch {}
      ...
      if (e3.code === "EEXIST") {
        try {
          let e4 = Se(Ne(Yt, "pid"), "utf8"), t2 = parseInt(e4.split(`
`)[1] ?? "0");
...
O(Qt, "acquireTokenRefreshLock"), O(Vt, "pollDiskForFreshToken");
var Xt = class {
  static {
    O(this, "ClaudeCodeSDK");
  }
  accessToken = null;
  refreshToken = null;
  expiresAt = null;
  ...
  $ = null;
  constructor(e2 = {}) {
    this.sessionId = ve(), this.deviceId = e2.deviceId ?? $e(32).toString("hex"), this.accountUuid = e2.accountUuid ?? this.readAccountUuid(), this.timeout = e2.timeout ?? 600000, this.maxRetries = e2.maxRetries ?? 10, this.onTokenStatus = e2.onTokenStatus, this.keepalive = new $t({ config: e2.keepalive, getToken: O(async () => (await this.ensureAuth(), this.accessToken ?? ""), "getToken"), doFetch: O((e3, t2, i2) => this.doStreamRequest(e3, t2, i2), "doFetch"), getRateLimitInfo: O(() => this.lastRateLimitInfo, "getRateLimitInfo") }), e2.credentialStore ? this.credentialStore = e2.credentialStore : e2.accessToken ? (this.accessToken = e2.accessToken, this.refreshToken = e2.refreshToken ?? null, this.expiresAt = e2.expiresAt ?? null, this.credentialStore = new ei({ accessToken: e2.accessToken, refreshToken: e2.refreshToken ?? "", expiresAt: e2.expiresAt ?? 0 }), this.expiresAt && this.refreshToken && this.scheduleProactiveRotation()) : (this.credentialStore = new Zt(e2.credentialsPath ?? Ne(xe(), ".claude", ".credentials.json")), this.initialLoad = this.loadFromStore().catch(() => {})), this.tokenRotation = new Nt(this.credentialStore, e2.contextTokensProvider, () => gt());
  }
  close() {
  ...
  async* stream(e2) {
    ...
    this.keepalive.notifyRealRequestStart(e2.model, i2, s2), this.$ = null;
    for (let r2 = 1;r2 <= this.maxRetries + 1; r2++) {
      if (e2.signal?.aborted)
        throw new Le("Aborted");
      try {
        return yield* this.doStreamRequest(i2, s2, e2.signal), void (this.$ && (this.keepalive.notifyRealRequestComplete(this.$), this.$ = null));
      } catch (i3) {
        if (t2 = i3, i3 instanceof He) {
          if (i3.status === 401 && r2 <= this.maxRetries) {
            await this.handleAuth401(), s2[f] = `Bearer ${this.accessToken}`;
            continue;
          }
          if (i3.status === 429)
            throw i3 instanceof Fe ? i3 : new Fe("Rate limited", this.lastRateLimitInfo, 429, i3);
  ...
  async* doStreamRequest(e2, t2, i2) {
    let s2 = new AbortController, r2 = setTimeout(() => s2.abort(), this.timeout);
    i2 && i2.addEventListener("abort", () => s2.abort(), { once: true });
    let n2, a2 = Date.now(), o2 = JSON.stringify(e2);
    if (o2.length > 19922944) {
      let t3 = (o2.length / 1024 / 1024).toFixed(1);
      try {
        let { appendFileSync: e3 } = M("fs");
        e3(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] BODY_TOO_LARGE pid=${process.pid} bodyLen=${o2.length} (${t3}MB) \u2014 refusing to send (would 413)
`);
      } catch {}
      let i3 = e2.messages?.length ?? 0;
      throw new Error(`Request body too large: ${t3}MB exceeds the 19MB safety threshold (API hard limit ~20MB). Got ${i3} messages. Run /compact to summarize history, or start a fresh session. If this is unexpected, the consumer's per-image / total-body management may have a gap.`);
    }
    try {
      let { appendFileSync: i3 } = M("fs");
      i3(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] API_START pid=${process.pid} model=${e2.model} msgs=${e2.messages?.length ?? 0}
`);
      let s3 = e2.tools?.map((e3) => e3.name).join(",") ?? "none", r3 = typeof e2.system == "string" ? e2.system.substring(0, 200) : JSON.stringify(e2.system)?.substring(0, 200);
      if (i3(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] API_REQ pid=${process.pid} headers=${JSON.stringify(t2).substring(0, 300)} tools=[${s3.substring(0, 500)}] sys=${r3} bodyLen=${o2.length}
`), process.env.CLAUDE_MAX_DUMP_REQUESTS === "1") {
        let s4 = { ...e2, messages: `[${e2.messages?.length ?? 0} messages]`, system: `[${typeof e2.system == "string" ? e2.system.length : "array"}]` };
        i3(Ne(xe(), ".claude", "claude-max-request-dump.jsonl"), JSON.stringify({ ts: new Date().toISOString(), pid: process.pid, headers: t2, body: s4 }) + `
`);
      }
      let n3 = true;
      try {
        let { readFlags: e3 } = (Y(), l2 = x, ((e4, t3, i4, s4) => {
          if (t3 && typeof t3 == "object" || typeof t3 == "function")
            for (let i5 of D(t3))
              !b.call(e4, i5) && i5 !== undefined && v(e4, i5, { get: () => t3[i5], enumerable: !(s4 = S(t3, i5)) || s4.enumerable });
          return e4;
        })(v({}, "__esModule", { value: true }), l2));
        n3 = e3().dump_bodies_full;
      } catch {
        n3 = process.env.CLAUDE_DUMP_BODIES_FULL === "1";
      }
      if (n3)
        try {
          let { mkdirSync: e3, writeFileSync: t3 } = M("fs"), s4 = Ne(xe(), ".claude", "body-dumps");
          e3(s4, { recursive: true });
          let r4 = globalThis.__claudeMaxBodyTurn = (globalThis.__claudeMaxBodyTurn ?? 0) + 1, n4 = `${process.pid}-${r4.toString().padStart(4, "0")}-${Date.now()}.json`;
          t3(Ne(s4, n4), o2), i3(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] BODY_DUMPED pid=${process.pid} turn=${r4} bytes=${o2.length} file=${n4}
`);
        } catch (e3) {
          i3(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] BODY_DUMP_FAILED pid=${process.pid} err=${e3?.message}
`);
        }
    } catch {}
    var l2;
    try {
      n2 = await fetch(`${xt}/v1/messages?beta=true`, { method: "POST", headers: t2, body: o2, signal: s2.signal });
    } catch (e3) {
      clearTimeout(r2);
      try {
        let { appendFileSync: t3 } = M("fs");
        t3(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] API_ERROR pid=${process.pid} ttfb=${Date.now() - a2}ms err=${e3.message}
`);
      } catch {}
      throw new Le("Network error", e3);
    }
    clearTimeout(r2);
    try {
      let { appendFileSync: e3 } = M("fs"), t3 = {};
      n2.headers.forEach((e4, i4) => {
        t3[i4] = e4;
      });
      let i3 = { ts: new Date().toISOString(), pid: process.pid, status: n2.status, statusText: n2.statusText, ttfbMs: Date.now() - a2, headers: t3 };
      e3(Ne(xe(), ".claude", "claude-max-api-responses.log"), JSON.stringify(i3) + `
`), e3(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] API_RESPONSE pid=${process.pid} status=${n2.status} ttfb=${Date.now() - a2}ms
`);
    } catch {}
    if (this.lastRateLimitInfo = this.parseRateLimitHeaders(n2.headers), !n2.ok) {
      ...
      let t3 = n2.headers.get("request-id");
      try {
        let { appendFileSync: i3 } = M("fs"), s3 = {};
        n2.headers.forEach((e4, t4) => {
          s3[t4] = e4;
        }), i3(Ne(xe(), ".claude", "claude-max-api-responses.log"), JSON.stringify({ ts: new Date().toISOString(), pid: process.pid, type: "ERROR", status: n2.status, requestId: t3, headers: s3, body: e3.slice(0, 5000), rateLimitInfo: this.lastRateLimitInfo }) + `
`);
      } catch {}
      throw n2.status === 429 ? new Fe(`Rate limited: ${e3}`, this.lastRateLimitInfo, 429) : new He(`API error ${n2.status}: ${e3}`, n2.status, t3);
    }
    if (!n2.body)
      throw new Le("No response body");
    yield* this.parseSSE(n2.body, i2);
  }
  async* parseSSE(e2, t2) {
    let i2 = new TextDecoder, s2 = e2.getReader(), r2 = "", n2 = new Map, a2 = { inputTokens: 0, outputTokens: 0 }, o2 = null;
    try {
      for (;; ) {
        if (t2?.aborted)
          return void s2.cancel();
        let { done: e3, value: l2 } = await s2.read();
        if (e3)
          break;
        r2 += i2.decode(l2, { stream: true });
        let h2 = r2.split(`
`);
        r2 = h2.pop() ?? "";
        for (let e4 of h2) {
          ...
          if (s3 === "message_start") {
            try {
              let { appendFileSync: e6 } = M("fs"), { join: i4 } = M("path"), { homedir: s4 } = M("os");
              e6(i4(s4(), ".claude", "claude-max-headers.log"), `[${new Date().toISOString()}] MESSAGE_START: ${JSON.stringify(t3).slice(0, 2000)}
`);
            } catch {}
            let e5 = t3.message?.usage;
            if (e5) {
              a2 = { inputTokens: e5.input_tokens ?? 0, outputTokens: e5.output_tokens ?? 0, cacheCreationInputTokens: e5.cache_creation_input_tokens, cacheReadInputTokens: e5.cache_read_input_tokens };
              let t4 = e5.cache_creation;
              t4 && typeof t4 == "object" && (typeof t4.ephemeral_5m_input_tokens == "number" && (a2.cacheCreation5mInputTokens = t4.ephemeral_5m_input_tokens), typeof t4.ephemeral_1h_input_tokens == "number" && (a2.cacheCreation1hInputTokens = t4.ephemeral_1h_input_tokens)), typeof e5.cache_deleted_input_tokens == "number" && (a2.cacheDeletedInputTokens = e5.cache_deleted_input_tokens);
              try {
                Ce(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] RAW_USAGE: ${JSON.stringify(e5)}
`);
              } catch {}
            }
            continue;
          }
          if (s3 === "content_block_start") {
            let { index: e5, content_block: i4 } = t3;
            if (i4.type === "tool_use") {
              let t4 = zt(i4.name);
              n2.set(e5, { type: "tool_use", id: i4.id, name: t4, input: "" }), yield { type: "tool_use_start", id: i4.id, name: t4 };
            } else
              i4.type === "text" ? n2.set(e5, { type: "text", text: "" }) : i4.type === "thinking" && n2.set(e5, { type: "thinking", thinking: "", signature: i4.signature ?? undefined });
            continue;
          }
          if (s3 === "content_block_delta") {
            let e5 = t3.index, i4 = n2.get(e5), s4 = t3.delta;
            s4.type === "text_delta" && s4.text !== undefined ? (i4 && (i4.text = (i4.text ?? "") + s4.text), s4.text && (yield { type: "text_delta", text: s4.text })) : s4.type === "thinking_delta" && s4.thinking !== undefined ? (i4 && (i4.thinking = (i4.thinking ?? "") + s4.thinking), s4.thinking && (yield { type: "thinking_delta", text: s4.thinking })) : s4.type === "signature_delta" && s4.signature !== undefined ? i4 && (i4.signature = (i4.signature ?? "") + s4.signature) : s4.type === "input_json_delta" && s4.partial_json !== undefined && (i4 && (i4.input = (i4.input ?? "") + s4.partial_json), s4.partial_json && (yield { type: "tool_use_delta", partialInput: s4.partial_json }));
            continue;
          }
          if (s3 === "content_block_stop") {
            let e5 = t3.index, i4 = n2.get(e5);
          ...
          if (s3 === "message_delta") {
            let e5 = t3.delta;
            e5?.stop_reason && (o2 = e5.stop_reason);
            let i4 = t3.usage;
            i4?.output_tokens && (a2 = { ...a2, outputTokens: i4.output_tokens });
            continue;
          }
          s3 === "message_stop" && (this.$ = a2, yield { type: "message_stop", usage: a2, stopReason: o2 });
        }
      }
    } finally {
      s2.releaseLock();
  ...
  buildHeaders(e2) {
    ...
    return { [d]: E, [f]: `Bearer ${this.accessToken}`, [y]: Lt, [g]: t2.join(","), [w]: "true", [_]: "cli", [m]: `claude-cli/${Wt}`, [T]: this.sessionId };
  }
  buildRequestBody(e2) {
    let t2, i2 = this.computeFingerprint(e2.messages), s2 = `x-anthropic-billing-header: cc_version=${Wt}.${i2}; cc_entrypoint=cli; cch=00000;`;
...
` + e2.system : Array.isArray(e2.system) ? [{ type: "text", text: s2 }, ...e2.system] : s2;
    let r2 = { model: e2.model, messages: e2.messages, max_tokens: Je(e2.model, e2.maxTokens), stream: true, system: t2, metadata: { user_id: JSON.stringify({ device_id: this.deviceId, account_uuid: this.accountUuid, session_id: this.sessionId }) } };
    if (e2.tools && e2.tools.length > 0) {
      let { remapped: t3 } = Gt(e2.tools);
      if (r2.tools = t3, e2.toolChoice) {
        let t4 = typeof e2.toolChoice == "string" ? { type: e2.toolChoice } : { ...e2.toolChoice };
        t4.type === "tool" && t4.name && jt[t4.name] && (t4.name = jt[t4.name]), r2.tool_choice = t4;
      }
    }
    ...
    if ((await this.tokenRotation.checkPending()).action === "apply-now" && (this.accessToken = null, this.refreshToken = null, this.expiresAt = 0), this.accessToken && this.credentialStore.hasChanged)
      try {
        if (await this.credentialStore.hasChanged()) {
          try {
            let e2 = vt(this.accessToken);
            Ce(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] TOKEN_FILE_CHANGED pid=${process.pid} oldHint=${e2} reason=mtime_diff_in_fast_path action=invalidate_in_memory_token
`);
          } catch {}
        ...
        try {
          Ce(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] TOKEN_MTIME_CHECK_FAILED pid=${process.pid} error=${e2?.message ?? String(e2)}
`);
        } catch {}
      }
  ...
  async loadFromStore() {
    ...
    if (!t2?.accessToken)
      throw new Pe('No OAuth tokens found. Run "claude login" first or provide credentials.');
    ...
    try {
      ...
      if (e2)
        if (e2 !== t2.accessToken) {
          let t3 = vt(e2);
          Ce(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] TOKEN_LOADED pid=${process.pid} reason=rotation oldHint=${t3} newHint=${i2} expiresInSec=${Math.round((this.expiresAt - Date.now()) / 1000)}
`);
        } else
          Ce(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] TOKEN_LOADED pid=${process.pid} reason=reload hint=${i2}
`);
      else
        Ce(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] TOKEN_LOADED pid=${process.pid} reason=initial newHint=${i2} expiresInSec=${Math.round((this.expiresAt - Date.now()) / 1000)}
`);
    } catch {}
  }
  isTokenExpired() {
    return !!this.expiresAt && Date.now() + Ht >= this.expiresAt;
  }
  ...
  async forceReLogin() {
    ...
    try {
      let { oauthLogin: e2 } = await Promise.resolve().then(() => (ke(), Q)), t2 = this.credentialStore instanceof Zt ? this.credentialStore.path : Ne(xe(), ".claude", ".credentials.json"), i2 = await e2({ credentialsPath: t2 });
  ...
  getTokenHealth() {
    ...
    return e2 = i2 <= 0 ? "expired" : r2 < 0.1 ? "critical" : r2 < Ft ? "warning" : "healthy", { expiresAt: this.expiresAt, expiresInMs: i2, lifetimePct: r2, failedRefreshes: this.proactiveRefreshFailures, status: e2 };
  }
  ...
  scheduleProactiveRotation() {
    ...
    let i2 = Math.max(0.8 * t2, Ut), s2 = Math.floor(60000 * Math.random()), r2 = Math.min(i2 + s2, t2 - Ht);
    if (r2 <= 0)
  ...
  async proactiveRefresh() {
    if (this.isRefreshOnCooldown()) {
      try {
        let e3 = await this.credentialStore.read();
        if (e3 && !(Date.now() + Ht >= e3.expiresAt)) {
    ...
    let e2 = Date.now();
    if (e2 - this.lastRefreshAttemptAt < Ut)
    ...
    if (!t2) {
      ...
      let e3 = await Vt(this.credentialStore, 45000, Bt);
      if (e3) {
    ...
    try {
      ...
      if (this.dbg(`proactive rotation FAILED (#${this.proactiveRefreshFailures}): ${t3}`), t3.includes("429") || t3.includes("rate limit")) {
        ...
        let e4 = Math.min(Ut * Math.pow(2, this.refreshConsecutive429s), Jt);
      ...
      i2 <= Ht ? this.emitTokenStatus("expired", `Token expired after ${this.proactiveRefreshFailures} failed refresh attempts: ${t3}`) : r2 < 0.1 ? this.emitTokenStatus("critical", `CRITICAL: ${Math.round(i2 / 60000)}min left, ${this.proactiveRefreshFailures} failures. Last: ${t3}. Consider forceReLogin()`) : r2 < Ft && this.emitTokenStatus("warning", `WARNING: ${Math.round(i2 / 60000)}min left, ${this.proactiveRefreshFailures} failures. Last: ${t3}`), this.expiresAt && this.expiresAt > Date.now() + Ht ? this.scheduleProactiveRotation() : (this.dbg("proactive rotation: token nearly expired \u2014 emitting expired status"), this.emitTokenStatus("expired", `Token expired \u2014 refresh failed ${this.proactiveRefreshFailures} times. Call forceReLogin() to recover.`));
    } finally {
      t2 && t2();
    }
  }
  ...
  isRefreshOnCooldown() {
    try {
      let e2 = Se(Kt, "utf8"), t2 = parseInt(e2.trim());
      if (Date.now() < t2)
        return true;
      try {
        Ie(Kt);
...
`);
  ...
  dbg(e2) {
    try {
      Ce(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] TOKEN_ROTATION pid=${process.pid} ${e2}
`);
    } catch {}
  }
  async refreshTokenWithTripleCheck() {
    let e2 = await this.credentialStore.read();
    if (e2 && !(Date.now() + Ht >= e2.expiresAt))
    ...
    if (!t2) {
      ...
      let e3 = await Vt(this.credentialStore, 45000, Ht);
  ...
  async handleAuth401() {
    ...
    this.pending401 && this.lastFailedToken === e2 || (this.lastFailedToken = e2, this.pending401 = (async () => {
      ...
      if (!i2) {
        ...
        let t3 = await Vt(this.credentialStore, 45000, Ht);
      ...
      try {
        ...
        if (t3 && t3.accessToken !== e2 && !(Date.now() + Ht >= t3.expiresAt))
  ...
  async doTokenRefresh(e2 = false) {
    if (!this.refreshToken)
      throw new Pe("Token expired and no refresh token available.");
    if (this.isRefreshOnCooldown() && !e2) {
      ...
      if (this.expiresAt && this.expiresAt > Date.now() + 600000)
        throw new Pe("Token refresh on cooldown due to rate limiting. Will retry later.");
      this.dbg("refresh: ignoring cooldown \u2014 token critically close to expiry");
    }
    let t2 = [500, 1500, 3000, 5000, 8000], i2 = this.credentialStore.path ?? Ne(xe(), ".claude", ".credentials.json");
    try {
      let t3 = Ae(i2).mtimeMs, s3 = Date.now() - t3;
      if (s3 < 60000) {
        let t4 = await this.credentialStore.read();
        if (t4 && !(Date.now() + Ht >= t4.expiresAt)) {
    ...
    for (let i3 = 0;i3 < 5; i3++) {
      ...
      if (s3 && !(Date.now() + Ht >= s3.expiresAt)) {
        if (!e2)
        ...
        if (s3.accessToken !== this.accessToken && t3 >= Bt)
      ...
      let r2 = await fetch(Pt, { method: "POST", headers: { [d]: E }, body: JSON.stringify({ grant_type: "refresh_token", refresh_token: this.refreshToken, client_id: "9d1c250a-e61b-44d9-88ed-5944d1962f5e" }), signal: AbortSignal.timeout(15000) });
      if (r2.ok) {
      ...
      if (r2.status === 429) {
        let e3 = Math.min(60000, Jt);
        throw this.setRefreshCooldown(e3), this.dbg(`TOKEN_REFRESH_RETRY status=429 attempt=${i3 + 1}/5 \u2014 bailing out, cooldown ${e3}ms (per-token rate limit)`), new Pe("Token refresh rate-limited (429) \u2014 will pickup from disk or retry after cooldown");
      }
      ...
      throw new Pe(`Token refresh failed: ${r2.status} ${r2.statusText}`);
    }
    let s2 = await this.credentialStore.read();
    if (!s2 || Date.now() + Ht >= s2.expiresAt)
      throw new Pe("Token refresh failed after all retries and race recovery");
    this.accessToken = s2.accessToken, this.refreshToken = s2.refreshToken, this.expiresAt = s2.expiresAt;
    try {
      Ce(Ne(xe(), ".claude", "claude-max-debug.log"), `[${new Date().toISOString()}] TOKEN_REFRESH_RACE_RECOVERY pid=${process.pid}
`);
    } catch {}
  }
  ...
  parseRateLimitHeaders(e2) {
    ...
    if (e2.forEach((e3, i3) => {
      ...
      try {
        let { appendFileSync: e3 } = M("fs"), { join: i3 } = M("path"), { homedir: s3 } = M("os");
        e3(i3(s3(), ".claude", "claude-max-headers.log"), `[${new Date().toISOString()}] ${JSON.stringify(t2)}
`);
      } catch {}
    let i2 = e2.get("retry-after"), s2 = e2.get("anthropic-ratelimit-unified-reset"), r2 = s2 ? Number(s2) : null, n2 = e2.get("anthropic-ratelimit-unified-5h-utilization"), a2 = e2.get("anthropic-ratelimit-unified-7d-utilization");
    return { status: e2.get("anthropic-ratelimit-unified-status"), resetAt: Number.isFinite(r2) ? r2 : null, claim: e2.get("anthropic-ratelimit-unified-representative-claim"), retryAfter: i2 ? parseFloat(i2) : null, utilization5h: n2 ? parseFloat(n2) : null, utilization7d: a2 ? parseFloat(a2) : null };
  }
  getRetryDelay(e2, t2) {
    if (t2) {
  ...
  sleep(e2, t2) {
    return new Promise((i2, s2) => {
      if (t2?.aborted)
        return void s2(new Le("Aborted"));
      let r2 = setTimeout(i2, e2);
      t2?.addEventListener("abort", () => {
        clearTimeout(r2), s2(new Le("Aborted"));
      }, { once: true });
    });
  }
  computeFingerprint(e2) {
    ...
    let i2 = `59cf53e54c78${[4, 7, 20].map((e3) => t2[e3] || "0").join("")}${Wt}`;
    return Re("sha256").update(i2).digest("hex").slice(0, 3);
  }
  readAccountUuid() {
    try {
      let e2 = Ne(xe(), ".claude", "claude_code_config.json");
      return JSON.parse(Se(e2, "utf8")).oauthAccount?.accountUuid ?? "";
    } catch {
      return "";
    }
  }
...
var Zt = class {
  ...
  static {
    O(this, "FileCredentialStore");
  }
  lastMtimeMs = 0;
  async read() {
    try {
      let e2 = Se(this.path, "utf8");
  ...
  async write(e2) {
    let t2 = {};
    try {
      t2 = JSON.parse(Se(this.path, "utf8"));
    } catch {}
    t2.claudeAiOauth = e2;
    let i2 = Ne(this.path, "..");
    try {
      Oe(i2, { recursive: true });
    } catch {}
    De(this.path, JSON.stringify(t2, null, 2), "utf8"), be(this.path, 384), this.lastMtimeMs = this.getMtime();
  }
  async hasChanged() {
...
var ti = class _Conversation {
  ...
  rewind(e2) {
    ...
    return this.S.splice(e2);
  }
  undoLastTurn() {
    for (let e2 = this.S.length - 1;e2 >= 0; e2--) {
      let t2 = this.S[e2];
      if (t2.role === "user") {
        let i2 = t2.content;
  ...
  branch() {
    ...
    return e2.S = [...this.S], e2.D = { ...this.D }, e2;
  }
  getHistory() {
    return this.S.map((e2, t2) => {
      let i2 = "";
  ...
  appendAssistantFromResponse(e2) {
    ...
    t2.length > 0 && this.S.push({ role: "assistant", content: t2 });
  }
  ...
  accumulateUsage(e2) {
    this.D.inputTokens += e2.inputTokens, this.D.outputTokens += e2.outputTokens, this.D.cacheCreationInputTokens = (this.D.cacheCreationInputTokens ?? 0) + (e2.cacheCreationInputTokens ?? 0), this.D.cacheReadInputTokens = (this.D.cacheReadInputTokens ?? 0) + (e2.cacheReadInputTokens ?? 0);
  }
};
function oi(e2, t2) {
  ri(ni(e2), { recursive: true });
  let i2 = null, s2 = [];
  for (let e3 of t2) {
    let t3 = ai(), r2 = { type: e3.role === "user" ? "user" : "assistant", uuid: t3, parentUuid: i2, timestamp: Date.now(), content: e3.content };
...
O(oi, "saveSession"), O(li, "loadSession");
var hi = class {
  static {
    O(this, "CacheMetricsCollector");
  }
  samples = [];
  timer = null;
  previousHitRate = 1;
...
var di = class {
  static {
    O(this, "FileCredentialsProvider");
  }
  path;
  expiryBufferMs;
  cached = null;
  lastMtimeMs = 0;
  constructor(e2 = {}) {
    this.path = e2.path ?? fi(), this.expiryBufferMs = e2.expiryBufferMs ?? 300000;
  }
...
O(fi, "defaultCredentialsPath");
var pi = { error: 0, info: 1, debug: 2 };
var mi = class {
  static {
    O(this, "ConsoleEventEmitter");
  }
  minRank;
  format;
  write;
  constructor(e2 = {}) {
    this.minRank = pi[e2.minLevel ?? "info"] ?? 1, this.format = e2.format ?? "human", this.write = e2.writeTarget ?? ((e3) => process.stderr.write(e3 + `
`));
  }
  emit(e2) {
    try {
      if ((pi[e2.level] ?? 1) > this.minRank)
        return;
...
var yi = class {
  static {
    O(this, "NullEventEmitter");
  }
  emit(e2) {}
};
var gi = class {
  static {
    O(this, "InMemorySessionStore");
  }
  sessions = new Map;
  liveness;
  constructor(e2 = new wi) {
    this.liveness = e2;
  }
...
C(), N();
var Ti = { anthropicBaseUrl: t, kaIntervalSec: undefined, kaIdleTimeoutSec: 0, kaMinTokens: 2000, kaRewriteWarnIdleSec: 300, kaRewriteWarnTokens: 50000, kaRewriteBlockIdleSec: 0, kaRewriteBlockEnabled: false };
var Ei = class {
  static {
    O(this, "ProxyClient");
  }
  ...
  constructor(e2) {
    this.config = { ...Ti, ...e2.config }, this.credentials = e2.credentialsProvider, this.events = e2.eventEmitter ?? new mi, this.liveness = e2.livenessChecker ?? new wi, this.store = e2.sessionStore ?? new gi(this.liveness), this.upstream = e2.upstreamFetcher ?? new _i, this.metrics = new hi({ windowMs: 60000, reportIntervalMs: 60000, onSummary: O((e3) => this.events.emit({ level: "info", kind: "CACHE_METRICS_SUMMARY", ...e3 }), "onSummary"), onRegression: O((e3) => this.events.emit({ level: "error", kind: "CACHE_REGRESSION_DETECTED", ...e3 }), "onRegression") }), this.reaperTimer = setInterval(() => {
  ...
  async handleRequest(e2, t2, i2) {
    ...
    try {
      ...
      return this.events.emit({ level: "error", kind: "REAL_REQUEST_ERROR", sessionId: s2, msg: "Invalid JSON body" }), vi(400, { error: "Invalid JSON" });
    }
    ...
    try {
      ...
      return this.events.emit({ level: "error", kind: "TOKEN_NEEDS_RELOGIN", sessionId: s2, msg: e3?.message ?? "No OAuth credentials" }), vi(401, { error: { type: "authentication_error", message: e3?.message ?? "No OAuth credentials" } });
    }
    let u2 = c2[g] ?? c2["Anthropic-Beta"] ?? "";
    if (!u2.includes("oauth-2025-04-20")) {
      let e3 = u2 ? u2 + "," : "";
      c2[g] = e3 + "oauth-2025-04-20", delete c2["Anthropic-Beta"];
    }
    ...
    try {
      ...
      if (e3?.code === "CACHE_REWRITE_BLOCKED")
        return vi(429, { error: { type: "cache_rewrite_blocked", message: e3.message } });
      throw e3;
    }
    let d2, p2, m2, y2 = Date.now();
    try {
    ...
    if (this.lastRateLimit = $i(d2.headers), !d2.ok) {
    ...
    this.parseSSEAndNotify(m2, n2, s2, h2, y2).catch((e3) => {
      this.events.emit({ level: "error", kind: "REAL_REQUEST_ERROR", sessionId: s2, msg: `parse promise rejected: ${e3?.message}` });
    });
    let w2 = new Headers(d2.headers);
    return w2.delete("content-encoding"), w2.delete("content-length"), new Response(p2, { status: d2.status, headers: w2 });
  }
  createEngine(e2) {
    let t2 = this.config;
    return new $t({ config: { intervalMs: t2.kaIntervalSec !== undefined ? 1000 * t2.kaIntervalSec : undefined, idleTimeoutMs: t2.kaIdleTimeoutSec > 0 ? 1000 * t2.kaIdleTimeoutSec : 1 / 0, minTokens: t2.kaMinTokens, rewriteWarnIdleMs: 1000 * t2.kaRewriteWarnIdleSec, rewriteWarnTokens: t2.kaRewriteWarnTokens, rewriteBlockIdleMs: t2.kaRewriteBlockIdleSec > 0 ? 1000 * t2.kaRewriteBlockIdleSec : 1 / 0, rewriteBlockEnabled: t2.kaRewriteBlockEnabled, onHeartbeat: O((t3) => {
    ...
    }, "onTick"), onDisarmed: O((t3) => this.events.emit({ level: "error", kind: "KA_DISARM", sessionId: e2, reason: t3.reason, msg: `KA disarmed for session ${e2.slice(0, 8)} \u2014 reason=${t3.reason}` }), "onDisarmed"), onRewriteWarning: O((t3) => this.events.emit({ level: t3.blocked ? "error" : "info", kind: t3.blocked ? "REWRITE_BLOCK" : "REWRITE_WARN", sessionId: e2, idleMs: t3.idleMs, estimatedTokens: t3.estimatedTokens, blocked: t3.blocked, model: t3.model }), "onRewriteWarning"), onNetworkStateChange: O((t3) => this.events.emit({ level: t3.to === "degraded" ? "error" : "info", kind: t3.to === "degraded" ? "NETWORK_DEGRADED" : "NETWORK_HEALTHY", sessionId: e2, from: t3.from, to: t3.to }), "onNetworkStateChange") }, getToken: O(() => this.credentials.getAccessToken(), "getToken"), doFetch: O((e3, t3, i2) => this.engineDoFetch(e3, t3, i2), "doFetch"), getRateLimitInfo: O(() => this.lastRateLimit, "getRateLimitInfo"), isOwnerAlive: O(() => this.store.isOwnerAlive(e2), "isOwnerAlive") });
  }
...
`);
          ...
          if (t4 !== "[DONE]")
            try {
              ...
              if (e5.type === "message_start" && e5.message?.usage) {
                ...
                let i3 = t5.cache_creation;
                i3 && typeof i3 == "object" && (typeof i3.ephemeral_5m_input_tokens == "number" && (n2.cacheCreation5mInputTokens = i3.ephemeral_5m_input_tokens), typeof i3.ephemeral_1h_input_tokens == "number" && (n2.cacheCreation1hInputTokens = i3.ephemeral_1h_input_tokens)), typeof t5.cache_deleted_input_tokens == "number" && (n2.cacheDeletedInputTokens = t5.cache_deleted_input_tokens);
              } else
  ...
  handleNetworkError(e2, t2) {
    let i2 = t2?.code ?? t2?.cause?.code ?? "", s2 = String(t2?.message ?? "").toLowerCase(), r2 = Ri.has(i2) || s2.includes("unable to connect") || s2.includes("failed to open socket") || s2.includes("connection refused") || s2.includes("network");
    return this.events.emit({ level: "error", kind: "REAL_REQUEST_ERROR", sessionId: e2, status: r2 ? 503 : 502, msg: `upstream fetch threw: ${i2 || ""} ${s2}`.trim().slice(0, 200) }), r2 ? new Response(JSON.stringify({ type: "error", error: { type: "overloaded_error", message: "Upstream network error \u2014 proxy cannot reach Anthropic. Retrying will help once network is restored." } }), { status: 503, headers: { [d]: E, "retry-after": "2" } }) : new Response(JSON.stringify({ type: "error", error: { type: "api_error", message: `Upstream request failed: ${s2 || i2 || "unknown"}` } }), { status: 502, headers: { [d]: E } });
  }
};
var ki = ["host", "content-length", "connection", "authorization", "accept-encoding"];
var Ri = new Set(["ECONNREFUSED", "ECONNRESET", "ETIMEDOUT", "ENETUNREACH", "ENOTFOUND", "EAI_AGAIN", "UND_ERR_SOCKET", "UND_ERR_CONNECT_TIMEOUT"]);
function $i(e2) {
...
function vi(e2, t2) {
  return new Response(JSON.stringify(t2), { status: e2, headers: { [d]: E } });
}
      ...
      for (let e4 of o2) {
        ...
        if (i3 !== "[DONE]") {
          ...
          if (t3.type === "message_start" && t3.message?.usage) {
            ...
            let i4 = e5.cache_creation;
            i4 && typeof i4 == "object" && (typeof i4.ephemeral_5m_input_tokens == "number" && (n2.cacheCreation5mInputTokens = i4.ephemeral_5m_input_tokens), typeof i4.ephemeral_1h_input_tokens == "number" && (n2.cacheCreation1hInputTokens = i4.ephemeral_1h_input_tokens)), typeof e5.cache_deleted_input_tokens == "number" && (n2.cacheDeletedInputTokens = e5.cache_deleted_input_tokens);
          } else
...
O($i, "parseRateLimitHeaders"), O(vi, "jsonResponse"), O(Si, "parseSSEToEvents"), ke(), ke(), C(), N(), C(), N();
var Ii = '{"type":"KeepAlive"}';
var Ci = 16000;
var Ni = Math.floor(3200);
async function xi(e2, i2, s2) {
  let r2 = s2?.baseUrl ?? t, n2 = new URLSearchParams({ encoding: "linear16", sample_rate: String(Ci), channels: String(1), endpointing_ms: "300", utterance_end_ms: "1000", language: s2?.language ?? "en" });
  if (s2?.keyterms?.length)
    for (let e3 of s2.keyterms)
      n2.append("keyterms", e3);
  let a2 = `/api/ws/speech_to_text/voice_stream?${n2.toString()}`, o2 = Mi(16).toString("base64"), l2 = null, h2 = false, c2 = false, u2 = false, d2 = null, p2 = null, y2 = "", g2 = await new Promise((t2, i3) => {
    let s3 = setTimeout(() => {
      i3(new Error("voice_stream WebSocket connection timeout (10s)"));
    }, 1e4), n3 = new URL(r2), l3 = Oi({ hostname: n3.hostname, port: n3.port || 443, path: a2, method: "GET", headers: { [f]: `Bearer ${e2}`, [m]: "claude-cli/1.0.0 (subscriber, cli)", [_]: "cli", Connection: "Upgrade", Upgrade: "websocket", "Sec-WebSocket-Version": "13", "Sec-WebSocket-Key": o2 } });
    l3.on("upgrade", (e3, r3, n4) => {
      clearTimeout(s3);
      let a3 = Ai("sha1").update(o2 + "258EAFA5-E914-47DA-95CA-5AB5DC11E5B3").digest("base64");
      if (e3.headers["sec-websocket-accept"] !== a3)
        return r3.destroy(), void i3(new Error("WebSocket handshake failed: invalid accept header"));
      t2(r3);
    }), l3.on("response", (e3) => {
      if (e3.statusCode === 101 && e3.socket)
        return clearTimeout(s3), void t2(e3.socket);
      clearTimeout(s3), i3(new Error(`WebSocket upgrade rejected: HTTP ${e3.statusCode}`));
    }), l3.on("error", (e3) => {
      clearTimeout(s3), i3(new Error(`voice_stream connection failed: ${e3.message}`));
    }), l3.end();
  });
  function w2(e3) {
  ...
  function k2(e3, t2) {
    if (g2.destroyed)
      return;
    let i3, s3 = Mi(4), r3 = Buffer.alloc(e3.length);
    for (let t3 = 0;t3 < e3.length; t3++)
      r3[t3] = e3[t3] ^ s3[t3 % 4];
    e3.length < 126 ? (i3 = Buffer.alloc(6), i3[0] = 128 | t2, i3[1] = 128 | e3.length, s3.copy(i3, 2)) : e3.length < 65536 ? (i3 = Buffer.alloc(8), i3[0] = 128 | t2, i3[1] = 254, i3.writeUInt16BE(e3.length, 2), s3.copy(i3, 4)) : (i3 = Buffer.alloc(14), i3[0] = 128 | t2, i3[1] = 255, i3.writeBigUInt64BE(BigInt(e3.length), 2), s3.copy(i3, 10)), g2.write(Buffer.concat([i3, r3]));
  }
  h2 = true, O(w2, "wsSendText"), O(T2, "wsSendBinary"), O(E2, "wsSendClose"), O(k2, "wsSendFrame");
  let R2 = Buffer.alloc(0);
  function $2() {
    for (;R2.length >= 2; ) {
      let e3 = R2[0], t2 = R2[1], i3 = 15 & e3, s3 = !!(128 & t2), r3 = 127 & t2, n3 = 2;
      if (r3 === 126) {
        if (R2.length < 4)
          return;
        r3 = R2.readUInt16BE(2), n3 = 4;
      } else if (r3 === 127) {
        if (R2.length < 10)
          return;
        r3 = Number(R2.readBigUInt64BE(2)), n3 = 10;
      }
      s3 && (n3 += 4);
      let a3 = n3 + r3;
      if (R2.length < a3)
        return;
      let o3 = R2.subarray(n3, a3);
      if (s3) {
        let e4 = R2.subarray(n3 - 4, n3);
        o3 = Buffer.from(o3);
        for (let t3 = 0;t3 < o3.length; t3++)
          o3[t3] = o3[t3] ^ e4[t3 % 4];
      }
      if (R2 = R2.subarray(a3), i3 === 1)
        v2(o3.toString("utf8"));
      else {
        if (i3 === 8)
          return void S2(o3.length >= 2 ? o3.readUInt16BE(0) : 1005, o3.length > 2 ? o3.subarray(2).toString("utf8") : "");
        i3 === 9 && k2(o3, 10);
      }
    }
  }
  function v2(e3) {
    ...
    switch (t2.type) {
      case "TranscriptText": {
        let e4 = t2.data;
        c2 && p2?.(), e4 && (y2 = e4, i2.onTranscript(e4, false));
        break;
      }
      case "TranscriptEndpoint": {
        let e4 = y2;
        y2 = "", e4 && i2.onTranscript(e4, true), c2 && d2?.("post_closestream_endpoint");
        break;
      }
      case "TranscriptError": {
        let e4 = t2.description ?? t2.error_code ?? "unknown transcription error";
        u2 || i2.onError(e4);
        break;
      }
      case "error": {
        let e4 = t2.message ?? JSON.stringify(t2);
        u2 || i2.onError(e4);
        break;
      }
    }
  ...
  function S2(e3, t2) {
    if (h2 = false, l2 && (clearInterval(l2), l2 = null), y2) {
      let e4 = y2;
      y2 = "", i2.onTranscript(e4, true);
    }
    d2?.("ws_close"), !u2 && e3 !== 1000 && e3 !== 1005 && i2.onError(`Connection closed: code ${e3}${t2 ? ` \u2014 ${t2}` : ""}`), i2.onClose(), g2.destroy();
  }
  return O($2, "processFrames"), O(v2, "handleMessage"), O(S2, "handleClose"), g2.on("data", (e3) => {
    R2 = Buffer.concat([R2, e3]), $2();
  }), g2.on("close", () => {
    h2 && S2(1006, "connection lost");
  }), g2.on("error", (e3) => {
    u2 || i2.onError(`Socket error: ${e3.message}`);
  }), w2(Ii), l2 = setInterval(() => {
    h2 && w2(Ii);
  }, 8000), { send(e3) {
    !h2 || c2 || T2(Buffer.from(e3));
  }, finalize: () => u2 || c2 ? Promise.resolve("already_closed") : (u2 = true, new Promise((e3) => {
    let t2 = setTimeout(() => d2?.("safety_timeout"), 5000), s3 = setTimeout(() => d2?.("no_data_timeout"), 1500);
    p2 = O(() => {
      clearTimeout(s3), p2 = null;
    }, "cancelNoDataTimer"), d2 = O((r3) => {
      if (clearTimeout(t2), clearTimeout(s3), d2 = null, p2 = null, y2) {
        let e4 = y2;
        y2 = "", i2.onTranscript(e4, true);
      }
      e3(r3);
    }, "resolveFinalize"), g2.destroyed ? d2("ws_already_closed") : setTimeout(() => {
      c2 = true, h2 && w2('{"type":"CloseStream"}');
    }, 0);
  })), close() {
    c2 = true, l2 && (clearInterval(l2), l2 = null), h2 = false, g2.destroyed || (E2(), g2.destroy());
  }, isConnected: () => h2 && !g2.destroyed };
}
async function Li(e2, t2, i2) {
  let s2 = [], r2 = null, n2 = await xi(e2, { onTranscript: O((e3, t3) => {
    t3 ? s2.push(e3.trim()) : i2?.onInterim?.(e3);
  }, "onTranscript"), onError: O((e3) => {
    r2 = e3;
  }, "onError"), onClose: O(() => {}, "onClose") }, i2);
  try {
    let e3 = await Ki(t2), s3 = e3;
    e3.length > 44 && e3[0] === 82 && e3[1] === 73 && e3[2] === 70 && e3[3] === 70 && (s3 = e3.subarray(44));
    let r3 = i2?.realtime !== false;
    for (let e4 = 0;e4 < s3.length && n2.isConnected(); e4 += Ni) {
      let t3 = s3.subarray(e4, Math.min(e4 + Ni, s3.length));
      n2.send(t3), r3 && e4 + Ni < s3.length && await Bi(80);
    }
    await n2.finalize();
  } finally {
    n2.close();
  ...
  if (r2)
    throw new Error(`Transcription error: ${r2}`);
  return s2.join(" ");
}
async function Pi(e2, t2, i2) {
  let s2 = [], r2 = null, n2 = await xi(e2, { onTranscript: O((e3, t3) => {
    t3 ? s2.push(e3.trim()) : i2?.onInterim?.(e3);
  }, "onTranscript"), onError: O((e3) => {
    r2 = e3;
  ...
  try {
    ...
    await Wi(n2, t2, e3, i2?.realtime !== false), await n2.finalize();
  } finally {
    n2.close();
  }
  if (r2)
...
function Hi(e2, t2) {
  if (Ui("rec")) {
    let i2 = Di("rec", ["-q", "--buffer", "1024", "-t", "raw", "-r", String(Ci), "-e", "signed", "-b", String(16), "-c", String(1), "-", "silence", "1", "0.1", "3%", "1", "2.0", "3%"], { stdio: ["pipe", "pipe", "pipe"] });
    return i2.stdout?.on("data", e2), i2.stderr?.on("data", () => {}), i2.on("close", t2), i2.on("error", t2), { stop() {
      i2.kill("SIGTERM");
    } };
  }
  if (Ui("arecord")) {
    let i2 = Di("arecord", ["-f", "S16_LE", "-r", String(Ci), "-c", String(1), "-t", "raw", "-q", "-"], { stdio: ["pipe", "pipe", "pipe"] });
    return i2.stdout?.on("data", e2), i2.stderr?.on("data", () => {}), i2.on("close", t2), i2.on("error", t2), { stop() {
      i2.kill("SIGTERM");
    } };
  }
...
function Fi() {
  return Ui("rec") ? { available: true, tool: "sox", installHint: null } : Ui("arecord") ? { available: true, tool: "arecord", installHint: null } : { available: false, tool: null, installHint: { darwin: "brew install sox", linux: "sudo apt-get install sox  # or: sudo apt-get install alsa-utils" }[process.platform] ?? "Install SoX (sox) or ALSA utils (arecord)" };
}
function Ui(e2) {
  return bi(e2, ["--version"], { stdio: "ignore", timeout: 3000 }).error === undefined;
}
function Bi(e2) {
...
function Ji() {
  return Ui("ffmpeg") ? "ffmpeg" : Ui("sox") ? "sox" : null;
}
async function Wi(e2, t2, i2, s2) {
  let r2 = i2 === "ffmpeg" ? ["-i", t2, "-f", "s16le", "-ar", String(Ci), "-ac", String(1), "pipe:1"] : [t2, "-t", "raw", "-r", String(Ci), "-e", "signed", "-b", String(16), "-c", String(1), "-"], n2 = Di(i2, r2, { stdio: ["pipe", "pipe", "pipe"] });
    ...
    n2.stdout?.on("data", async (t4) => {
      if (e2.isConnected()) {
        if (e2.send(t4), s2) {
          ...
          s3 > 10 && (n2.stdout?.pause(), await Bi(s3), n2.stdout?.resume()), a2 = Date.now();
        }
      } else
        n2.kill("SIGTERM");
...
O(xi, "connectVoiceStream"), O(Li, "transcribeFile"), O(Pi, "transcribeAudioFile"), O(Hi, "startMicRecording"), O(Fi, "checkVoiceDeps"), O(Ui, "hasCommand"), O(Bi, "sleep"), O(Ki, "readFileAsBuffer"), O(Ji, "findConverter"), O(Wi, "streamConvertedAudio");
  ...
  server: async (input) => {
    ...
    return {
      config: async (config) => {
        ...
        for (const [id, info] of Object.entries(Be)) {
          const isAdaptive = je(id);
          config.provider["claude-max"].models[id] = {
            id,
            name: `${info.name} (Max)`,
            api: { id, url: t, npm: providerPath },
              ...
              const server = Bun.serve({
                ...
                fetch(req) {
                  ...
                  if (error) {
                    ...
                    return new Response("<h1>Login failed</h1>", { status: 400, headers: { [d]: R } });
                  }
                  if (!code || st2 !== state) {
