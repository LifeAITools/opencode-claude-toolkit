...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync3, mkdirSync as mkdirSync6, chmodSync, existsSync as existsSync6, statSync as statSync2 } from "fs";
...
// wake-preferences.ts
init_wake_types();
import { readFileSync as readFileSync4, writeFileSync as writeFileSync2, mkdirSync as mkdirSync5, renameSync as renameSync3, existsSync as existsSync5 } from "fs";
...
class CredentialManager {
  ...
  saveToDisk() {
    ...
    const dir = dirname3(this.credPath);
    try {
      mkdirSync6(dir, { recursive: true });
    } catch {}
    writeFileSync3(this.credPath, JSON.stringify(existing, null, 2), "utf8");
    try {
  ...
  server: async (input) => {
    ...
    return {
      config: async (config) => {
        ...
        for (const [id, info2] of Object.entries(MAX_MODELS)) {
          const is46 = id.includes("opus-4-6") || id.includes("sonnet-4-6") || id.includes("opus-4-7") || id.includes("sonnet-4-7");
          config.provider["claude-max"].models[id] = {
            ...
            limit: { context: info2.context, output: info2.output },
            status: "active",
            options: {},
            headers: {},
            ...is46 ? {
