    ...
    try {
      ...
      this.config.onHeartbeat?.({
        ...
        rateLimit: {
          ...
          claim: rl.claim,
          resetAt: rl.resetAt,
          // 🔴 САМИ ПОКАЗАНИЯ — то, ради чего этот отчёт вообще нужен ночью.
          // Раньше здесь стояли только три поля, и 31 506 служебных ударов
          // ушли в журнал без единого показания счётчика (замер 29.08.2026).
          // Ночь — единственное время, когда идут ТОЛЬКО эти удары, поэтому
          // без них нельзя ответить, тратит ли прогрев квоту: на настоящих
          // ходах показание всегда смешано с работой самого хода.
          utilization5h: rl.utilization5h,
          utilization7d: rl.utilization7d,
        },
      })
