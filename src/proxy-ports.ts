...
export interface Session<EngineT = unknown> {
  ...
  engine: EngineT
  model: string | null
  // Mirrors TokenUsage shape (src/types.ts). Cache fields are optional
  // because TokenUsage forwards `undefined` for unreported subfields
  // (Phase 3.B: cacheCreation5m/1h/Deleted) — keeps assignments type-clean
  // when proxy-client.ts assigns the parsed `usage` object verbatim.
  lastUsage: {
    inputTokens: number
    outputTokens: number
...
// ═══ Port 4: Upstream Fetcher (optional advanced extension) ════════
...
// ═══ Port 5: Liveness Checker (optional) ═══════════════════════════
...
// ═══ Bundled: everything a ProxyClient needs from adapters ════════
