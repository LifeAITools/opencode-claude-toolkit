...
export interface KeepaliveConfig {
   ...
   */
  intervalMs?: number
  idleTimeoutMs?: number     // default: Infinity (never auto-stop on idle)
  minTokens?: number         // default: 2000
...
export type ContentBlockParam =
  ...
  | { type: 'tool_result'; tool_use_id: string; content: string | ContentBlockParam[]; is_error?: boolean }
...
export type SystemParam = string | { type: 'text'; text: string }[]
...
export type StreamEvent =
  ...
  | { type: 'tool_use_delta'; partialInput: string }
  | { type: 'tool_use_end'; id: string; name: string; input: unknown }
  | { type: 'message_stop'; usage: TokenUsage; stopReason: string | null }
  | { type: 'error'; error: Error }

export interface TokenUsage {
  inputTokens: number
  outputTokens: number
  /** Сколько раз модель сама сходила искать в интернете за этот ход.
   *
   *  🔴 ДОБАВЛЕНО 05.09.2026, И ВОТ ПОЧЕМУ. Владелец чат-сервиса пришёл с
   *  вопросом фаундера: можно ли открыть людям поиск в сети в счёт подписки.
   *  Поиск через нас РАБОТАЕТ — проверено живой пробой, восемь ссылок в ответе.
   *  Но ответить, во что он обходится, было НЕЧЕМ: Anthropic возвращает число
   *  поисковых обращений отдельным полем `usage.server_tool_use`, а мы его не
   *  читали вовсе и в учёт не клали. То есть открыли бы людям трату, которую не
   *  видим.
   *
   *  Токены за поиск считаются как обычно, а САМИ ОБРАЩЕНИЯ — отдельная
   *  величина, и мерить её надо отдельным числом, а не пересчётом токенов. */
  webSearchRequests?: number
  /** То же для случая, когда модель сама открывает названную страницу. */
...
export class ClaudeCodeSDKError extends Error {
  constructor(message: string, public readonly cause?: unknown) {
    super(message)
    this.name = 'ClaudeCodeSDKError'
  }
}

export class AuthError extends ClaudeCodeSDKError {
...
export class APIError extends ClaudeCodeSDKError {
  constructor(
    message: string,
...
export class RateLimitError extends ClaudeCodeSDKError {
  constructor(
    message: string,
 ...
 */
export class CacheRewriteBlockedError extends ClaudeCodeSDKError {
  readonly code = 'CACHE_REWRITE_BLOCKED'
  constructor(public idleMs: number, public estimatedTokens: number, public model: string) {
    super(
      `CACHE_REWRITE_BLOCKED: session idle ${Math.round(idleMs / 1000)}s on model=${model}, ` +
      `next request would cost ~${estimatedTokens} cache_write tokens. ` +
      `Unset CLAUDE_MAX_REWRITE_BLOCK or raise CLAUDE_MAX_REWRITE_BLOCK_IDLE_SEC to proceed.`
    )
    this.name = 'CacheRewriteBlockedError'
  }
}
